...
"""
RAW_FEMS = (
    "[9] J.G. Emond-Rheault, A.T. Vincent, M.V. Trudel, J. Frey, M. Frenette, "
    "S.J. Charette AsaGEI2b: a new variant of a genomic island identified in the "
    "Aeromonas salmonicida subsp. salmonicida JF3224 strain isolated from a wild fish "
    "in Switzerland FEMS Microbiol Lett, 362 (2015) fnv093 Google Scholar"
)


class GenericParserTests(SimpleTestCase):
    def test_generic_extract(self) -> None:
        refs = extract_references("https://example.org/article/abc", GENERIC_SNIPPET)
        self.assertEqual(len(refs), 2)
        self.assertTrue(cast(str, refs[0]["doi"]).startswith("10.1126/science.1162986"))
        self.assertEqual(refs[1]["issued_year"], "2007")

    def test_raw_parser_best_effort(self) -> None:
        r = parse_raw_reference(RAW_FEMS)
        self.assertEqual(r.get("issued_year"), "2015")
        self.assertIn("FEMS Microbiol Lett", cast(str, r.get("container_title", "")))
        self.assertEqual(r.get("volume"), "362")
        self.assertIn("AsaGEI2b", cast(str, r.get("title", "")))

    def test_dedupe_by_doi(self) -> None:
        refs: list[dict[str, object]] = [
            {"raw": "x", "doi": "10.1/xyz", "issued_year": ""},
            {"raw": "x2", "doi": "https://doi.org/10.1/xyz", "issued_year": ""},
        ]
