...
(function () {
  ...
  function leftWidth() {
    return parseInt(localStorage.getItem("pc-left-w") || "260", 10) || 260;
  }
  function rightWidth() {
    return parseInt(localStorage.getItem("pc-right-w") || "360", 10) || 360;
  }

  // Use canonical helper to avoid drift
  function currentCollectionId() {
    try {
      return window.PCDOM?.currentCollectionId?.() || "all";
    } catch (_) {
      return "all";
    }
  }
  ...
  function makeExportMenu() {
    ...
    function openAt(x, y) {
      ...
      menu.dataset.viewsHref =
        "/collections/" + encodeURIComponent(col) + "/download-views.zip";
      ...
      try {
        window.PCDOM?.keepOnScreen?.(menu);
      } catch (_) {}
    }
    function close() {
      menu.style.display = "none";
    ...
    window.addEventListener("click", (e) => {
      if (!menu.contains(e.target) && e.target !== menu.anchorBtn) close();
    });
