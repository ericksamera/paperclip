...
async function fetchSummary(colId) {
  const r = await fetch(`/collections/${encodeURIComponent(colId)}/summary.json`, {
...
function writeMeta(el, j) {
  ...
  if (y.min != null && y.max != null)
    parts.push(
      `years <strong>${y.min}</strong>–<strong>${y.max}</strong> (span ${y.span})`
    );
...
function renderCharts(j) {
  ...
  new Chart(document.getElementById("pc-chart-years"), {
    type: "bar",
    data: {
      labels: labelsYears.reverse(),
      datasets: [{ data: valuesYears.reverse() }],
    },
