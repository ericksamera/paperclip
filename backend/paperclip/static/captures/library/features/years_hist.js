// services/server/paperclip/static/captures/library/features/years_hist.js
...
function findYearsGroup() {
  ...
  if (hint) {
    return (
      hint.closest(".z-group") ||
      hint.closest("[role='group'], [role='region']") ||
      hint.parentElement
    );
  }
...
/* ---------- extract year buckets from existing list ---------- */
function inferYearsFromGroup(group) {
  const host =
    group.querySelector(".z-list, .facet-list, [data-facet-list], ul, ol") || group;
...
function paintBars(chartEl, data, range) {
  ...
  data.forEach((d) => {
    ...
    if (
      range &&
      (range.min ?? -Infinity) <= d.year &&
      d.year <= (range.max ?? Infinity)
    ) {
...
function buildWidget(group) {
  ...
  const syncFromInputs = () => {
    let lo = clamp(
      parseInt($("input[data-role=min-year]", wrap).value, 10),
      minYear,
      maxYear
    );
    let hi = clamp(
      parseInt($("input[data-role=max-year]", wrap).value, 10),
      minYear,
      maxYear
    );
