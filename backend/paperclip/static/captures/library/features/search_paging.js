// services/server/paperclip/static/captures/library/features/search_paging.js
...
function rowHtml(r) {
  ...
  const doi = r.doi_url
    ? `<a href="${escapeHtml(r.doi_url)}" target="_blank" rel="noreferrer">${escapeHtml(
        r.doi || ""
      )}</a>`
    : "";
  const url = r.url
    ? `<div class="pc-link"><a href="${escapeHtml(
        r.url
      )}" target="_blank" rel="noreferrer">${escapeHtml(r.site_label || "")}</a></div>`
    : "";
  const journalCell = `<td class="pc-col-tight" data-col="journal" title="${escapeHtml(
    r.journal || ""
  )}">${escapeHtml(r.journal_short || "")}</td>`;
  const snippet = r.preview
    ? `<div class="pc-snippet">${escapeHtml(r.preview)}</div>`
    : "";
  return `
<tr class="pc-row" draggable="true" data-id="${r.id}" aria-selected="false"
  ...
  <td class="pc-col-tight" data-col="authors">
    <span class="pc-authors-inline" title="${authorsTitle}">${authorsTitle}</span>
  </td>
  ...
  <!-- placeholder to match the header gear column -->
...
function updateScrollStatus() {
  const el =
    document.getElementById("pc-scroll-status") ||
    (() => {
      const d = document.createElement("div");
      d.id = "pc-scroll-status";
      d.className = "pc-scroll-status";
      document.getElementById("z-shell")?.appendChild(d);
      return d;
    })();
...
async function maybeLoadMore() {
  ...
  const nearBottom =
    zCenter.scrollHeight - zCenter.scrollTop - zCenter.clientHeight < 600;
...
function initChips() {
  ...
  const colLabelFromDOM =
    document
      .querySelector(".z-left .z-group:first-child .z-link.active .z-label")
      ?.textContent?.trim() || null;

  if (params.year) {
    host.appendChild(
      chip(`Year: ${params.year}`, () => {
        location.search = buildQs({ year: null });
      })
    );
  }
  if (params.journal) {
    host.appendChild(
      chip("Journal", () => {
        location.search = buildQs({ journal: null });
      })
    );
  }
  if (params.site) {
    host.appendChild(
      chip(`Site: ${params.site}`, () => {
        location.search = buildQs({ site: null });
      })
    );
  }
  if (params.col) {
    host.appendChild(
      chip(`Collection: ${colLabelFromDOM || params.col}`, () => {
        location.search = buildQs({ col: null });
      })
    );
  }
