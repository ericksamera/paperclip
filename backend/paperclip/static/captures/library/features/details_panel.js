// services/server/paperclip/static/captures/library/features/details_panel.js
// Right-hand details panel for the selected rows.
// Uses canonical events (SELECTION, ROWS_CHANGED) via infra/events.

import { $, $$, on, escapeHtml } from "../infra/dom.js";
import { onRowsChanged, EVENTS } from "../infra/events.js";
...
function oneRowHtml(row) {
  ...
  const kwsHtml = kws.length
    ? `<div class="z-kws">${kws
        .map((k) => `<span class="z-kw">${escapeHtml(k)}</span>`)
        .join("")}</div>`
    : "";
  ...
  return `
    ...
    ${
      doi
        ? `<div class="z-meta"><a href="${escapeHtml(
            doiUrl
          )}" target="_blank" rel="noopener">${escapeHtml(doi)}</a></div>`
        : ""
    }
    ${
      abs
        ? `<div class="z-meta"><strong>Abstract.</strong> ${escapeHtml(
            truncate(abs, 700)
          )}</div>`
        : ""
    }
...
export function initDetailsPanel() {
  ...
  // Canonical rows-changed event (legacy → canonical bridge lives in infra/events)
