// services/server/paperclip/static/captures/library/features/collections_ctx_dnd.js
...
function selectedIds() {
  ...
  if (!ids.length) {
    ...
    if (tb) {
      tb.querySelectorAll(
        "tr.pc-row[aria-selected='true'], tr.pc-row.is-selected"
      ).forEach((tr) => {
        const id = rowId(tr);
        if (id) ids.push(id);
      });
...
async function assignIdsToCollection(ids, colId, op /* "add" | "remove" */) {
  ...
  toast({
    ...
    } collection failed (HTTP ${resp.status}).`,
  });
}
...
function wireRowDragEvents() {
  ...
  tb.addEventListener("dragstart", (e) => {
    ...
    if (!ids.length || !e.dataTransfer) return;
...
function addDndHandlersToCollectionAnchor(aEl, colId) {
  aEl.addEventListener("dragenter", (e) => {
    e.preventDefault();
    aEl.classList.add("dnd-over");
  });
  aEl.addEventListener("dragover", (e) => {
    e.preventDefault();
    if (e.dataTransfer) e.dataTransfer.dropEffect = "copy";
  });
  aEl.addEventListener("dragleave", () => aEl.classList.remove("dnd-over"));
  aEl.addEventListener("drop", async (e) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    try {
      ...
      if (obj?.type === "pc-ids" && Array.isArray(obj.ids) && obj.ids.length) {
        ids = obj.ids.map(String);
      }
    } catch {
      // ignore, fall back to selectedIds
    }
...
// Right-click on Collections (Open/Rename/Delete/dashboard)
let colMenuEl = null;
