...
class RunNowView(View):
    def post(self, request):
        ...
        queued = submit_analysis(run.id)
        messages.success(request, f"Analysis run {run.id} – {'QUEUED' if queued else 'STARTED'}")
...
class LatestGraphView(View):
    ...
    def get(self, request):
        ...
        if selected_id and run_qs.filter(pk=selected_id).exists():
            ...
            first = run_qs.first()
            run_id = int(first.id) if first else None
        embed_url = (reverse("analysis_graph_embed") + (f"?run={run_id}" if run_id else "")) if run_id else None
        latest = run_qs.first()
...
class GraphEmbedView(View):
    ...
    def get(self, request):
        ...
        run = AnalysisRun.objects.filter(pk=run_id).first() if run_id else AnalysisRun.objects.first()
        if not run:
...
class RunProgressView(View):
    def get(self, _request, pk: int):
        ...
        return JsonResponse({"ok": True, "id": run.id, "status": run.status, "progress": run.progress, "log": run.log})
