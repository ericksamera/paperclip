...
from .base import (
    ...
    collect_paragraphs_subtree, extract_from_li, augment_from_raw,
...
_NONCONTENT_RX = re.compile(
    r"\b(references?|acknowledg|author information|author contributions?|funding|ethics|competing interests?|data availability)\b",
    re.I,
)
...
def _extract_abstract(soup: BeautifulSoup) -> Optional[str]:
    ...
    for sec in soup.select("section#Abs1, section[aria-labelledby='Abs1'], section#abstract, section[aria-labelledby='abstract']"):
...
def _reference_items(soup: BeautifulSoup) -> List[Tag]:
    ...
    for sel in selectors:
        for li in soup.select(sel):
            ...
            items.append(li)
        if items:
            # Stop early if we already matched a strong selector
            if sel in (
                "div[data-container-section='references'] ol.c-article-references > li",
                "ol.c-article-references > li",
                "ol.c-article-references__list > li",
