...
from .base import DOI_RE, YEAR_RE, collapse_spaces, augment_from_raw, heading_text, dedupe_keep_order, dedupe_section_nodes

# -------------------------- small helpers --------------------------

_NONCONTENT_RX = re.compile(
    r"\b(references?|acknowledg|conflict of interest|funding|data availability|author contributions?)\b",
    re.I,
)
...
def _extract_abstract(soup: BeautifulSoup) -> Optional[str]:
    ...
    for host in soup.select("div#abstract, section#abstract, div[class*='abstract' i], section[class*='abstract' i]"):
    ...
    head = soup.find(lambda t: isinstance(t, Tag) and t.name in {"h2","h3","h4"} and re.search(r"\babstract\b", heading_text(t), re.I))
    if head:
        out = []
...
def _extract_sections(soup: BeautifulSoup) -> List[Dict[str, object]]:
    top_nodes = []
    for sec in soup.select("div.section.toc-section"):
        if sec.find_parent("div", class_=lambda cs: isinstance(cs, list) and "section" in cs and "toc-section" in cs):
            continue
...
def _apply_journal_alias(j: str) -> (str, str):
    js = j.strip().rstrip(".")
...
def parse_plos(_url: str, dom_html: str) -> List[Dict[str, object]]:
    ...
    for li in soup.select("ol.references li, ul.references li"):
        ...
        if YEAR_RE.search(raw):
            base["issued_year"] = YEAR_RE.search(raw).group(0)
