...
from .base import (
    DOI_RE, YEAR_RE, collapse_spaces, heading_text, dedupe_keep_order,
    paras_between, extract_from_li, augment_from_raw,
...
def _is_h2_section_title(h: Tag) -> bool:
    return isinstance(h, Tag) and h.name and h.name.lower() == "h2" and _has_class(
        h, "section-title", "js-splitscreen-section-title"
    )
...
def _is_abstract_h2(h: Tag) -> bool:
    return isinstance(h, Tag) and h.name and h.name.lower() == "h2" and (
        _has_class(h, "abstract-title") or re.search(r"\babstract\b", heading_text(h), re.I)
    )

def _next_sibling_heading(start: Tag, names: Tuple[str, ...]) -> Optional[Tag]:
...
def _iter_between(start: Tag, end: Optional[Tag]):
    cur = start.next_sibling
    while cur and cur is not end:
        yield cur
        cur = cur.next_sibling

def _collect_paragraphs_between(a: Tag, b: Optional[Tag]) -> List[str]:
...
def _first_content_h2(soup: BeautifulSoup) -> Optional[Tag]:
...
def _extract_sections(soup: BeautifulSoup) -> List[Dict[str, object]]:
    ...
    for i, h2 in enumerate(h2s):
        ...
        for node in _iter_between(h2, h2_end):
            if isinstance(node, Tag) and node.name and node.name.lower() in ("h3", "h4") and _is_hx_section_title(node):
        ...
        if parent_paras:
            node["paragraphs"] = parent_paras
        if children:
            node["children"] = children

        if node.get("paragraphs") or node.get("children"):
            out.append(node)

    # ----- Merge or create Introduction from headless lead-in -----
    if leadin_paras:
        intro_idx = next((i for i, n in enumerate(out) if (n.get("title") or "").strip().lower() == "introduction"), None)
        if intro_idx is not None:
            # Prepend lead-in to the real Introduction paragraphs
            prev = out[intro_idx].get("paragraphs") or []
...
def _strip_ref_noise(tag: Tag) -> None:
    ...
    for sel in [
        ".citation-links", ".crossref-doi", ".adsDoiReference", ".xslopenurl",
        ".worldcat-reference-ref-link", ".inst-open-url-holders"
    ]:
        for t in tag.select(sel):
            t.decompose()

def _extract_doi_from(tag: Tag) -> Optional[str]:
    # Prefer DOI anchors
...
def _extract_pmid_from(tag: Tag) -> Optional[str]:
