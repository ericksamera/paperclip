...
def _find_meta(content: str, names: tuple[str, ...]) -> str | None:
...
def _title_from_dom(dom_html: str) -> str | None:
    if not dom_html:
        return None
...
def _extract_abstract(dom_html: str) -> str | None:
    if not dom_html:
        return None
...
def _parse_authors(dom_html: str) -> list[str]:
    ...
    for m in re.finditer(
        r'<meta[^>]*\bname=["\']citation_author["\'][^>]*\bcontent=["\']([^"\']+)["\']',
        dom_html or "",
        re.I,
    ):
    ...
    m2 = re.search(
        r'<meta[^>]*\bname=["\']dc\.creator["\'][^>]*\bcontent=["\']([^"\']+)["\']',
        dom_html or "",
        re.I,
    )
    if m2:
        tail = html.unescape(m2.group(1))
...
def fallbacks(url: str | None, dom_html: str, content_html: str) -> dict[str, object]:
    ...
    - meta_updates includes: title, doi, issued_year (int), container_title, url,
    ...
    if issued_year is not None:
        meta_updates["issued_year"] = issued_year
    if container:
        meta_updates["container_title"] = container
    if url:
        meta_updates["url"] = url
    if keywords:
        meta_updates["keywords"] = keywords
    if authors:
        meta_updates["authors"] = authors
    return {
